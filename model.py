...
class DQN:
    ...
    def __init__(self, session, sample_signals, n_action):
        ...
        # 게임의 상태를 입력받을 변수
        # [신호 개수, 실수-허수부, 게임 상태의 갯수(현재+과거+과거..)]
        self.input_X = tf.placeholder(tf.float32, [None, self.sample_signals * 2, 2, self.STATE_LEN])
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, 32, [2, 2], padding='same', activation=tf.nn.relu)
    ...
    def remember(self, state, action, reward, terminal):
        ...
        next_state = np.reshape(state, (self.sample_signals * 2, 2, 1))
